<?xml version='1.0' encoding='UTF-8'?>
<Library LVVersion="10008000">
	<Property Name="NI.Lib.Icon" Type="Bin">%!#!!!!!!!)!"1!&amp;!!!-!%!!!@````]!!!!"!!%!!!(]!!!*Q(C=\&gt;7R=2MR%!81N=?"5Q&lt;/07RB7W!,&lt;'&amp;&lt;9+K1,7Q,&lt;)%N&lt;!NMA3X)DW?-RJ(JQ"I\%%Z,(@`BA#==ZB3RN;]28_,V7@PWW`:R`&gt;HV*SU_WE@\N_XF[3:^^TX\+2YP)D7K6;G-RV3P)R`ZS%=_]J'XP/5N&lt;XH,7V\SEJ?]Z#5P?=J4HP+5JTTFWS%0?=B$DD1G(R/.1==!IT.+D)`B':\B'2Z@9XC':XC':XBUC?%:HO%:HO&amp;R7QT0]!T0]!S0I4&lt;*&lt;)?=:XA-(]X40-X40-VDSGC?"GC4N9(&lt;)"D2,L;4ZGG?ZH%;T&gt;-]T&gt;-]T?.S.%`T.%`T.)^&lt;NF8J4@-YZ$S'C?)JHO)JHO)R&gt;"20]220]230[;*YCK=ASI2F=)1I.Z5/Z5PR&amp;)^@54T&amp;5TT&amp;5TQO&lt;5_INJ6Z;"[(H#&gt;ZEC&gt;ZEC&gt;Z$"(*ETT*ETT*9^B)HO2*HO2*(F.&amp;]C20]C2)GN4UE1:,.[:/+5A?0^NOS?UJ^3&lt;*\9B9GT@7JISVW7*NIFC&lt;)^:$D`5Q9TWE7)M@;V&amp;D,6;M29DVR]6#R],%GC47T9_/=@&gt;Z5V&gt;V57&gt;V5E&gt;V5(OV?^T[FTP?\`?YX7ZRP6\D=LH%_8S/U_E5R_-R$I&gt;$\0@\W/VW&lt;[_"\Y[X&amp;],0^^+,]T_J&gt;`J@_B_]'_.T`$KO.@I"XC-_N!!!!!!</Property>
	<Property Name="NI.Lib.Version" Type="Str">1.0.0.0</Property>
	<Property Name="NI.SortType" Type="Int">3</Property>
	<Item Name="Sorting" Type="Folder">
		<Item Name="Sorting" Type="Folder">
			<Item Name="_Subs" Type="Folder">
				<Item Name="Sort 2D String Array - Generate Format String.vi" Type="VI" URL="../Sorting/_Subs/Sort 2D String Array - Generate Format String.vi"/>
				<Item Name="Sort 2D String Array - Preview Sorted Array.vi" Type="VI" URL="../Sorting/_Subs/Sort 2D String Array - Preview Sorted Array.vi"/>
				<Item Name="Sort 2D String Array - Return Format String.vi" Type="VI" URL="../Sorting/_Subs/Sort 2D String Array - Return Format String.vi"/>
				<Item Name="Sort 2D String Array - Pre-process.vi" Type="VI" URL="../Sorting/_Subs/Sort 2D String Array - Pre-process.vi"/>
				<Item Name="Sort 2D String Array - StringTransform (Pre Sort).vi" Type="VI" URL="../Sorting/_Subs/Sort 2D String Array - StringTransform (Pre Sort).vi"/>
				<Item Name="Sort 2D String Array - String Transform (Post Sort).vi" Type="VI" URL="../Sorting/_Subs/Sort 2D String Array - String Transform (Post Sort).vi"/>
				<Item Name="Sort 2D String Array - Post-process.vi" Type="VI" URL="../Sorting/_Subs/Sort 2D String Array - Post-process.vi"/>
				<Item Name="Sort 2D String Array - String or Number.vi" Type="VI" URL="../Sorting/_Subs/Sort 2D String Array - String or Number.vi"/>
			</Item>
			<Item Name="Usage Examples" Type="Folder">
				<Item Name="Example - Sort 2D String Array.vi" Type="VI" URL="../Sorting/Usage Examples/Example - Sort 2D String Array.vi"/>
			</Item>
			<Item Name="_TypeDefs" Type="Folder">
				<Item Name="TYPE - Input Array Contains Header Row.ctl" Type="VI" URL="../Sorting/_TypeDefs/TYPE - Input Array Contains Header Row.ctl"/>
				<Item Name="TYPE - Sort Ascending.ctl" Type="VI" URL="../Sorting/_TypeDefs/TYPE - Sort Ascending.ctl"/>
				<Item Name="TYPE - Sorted Array (Cols in Original Order. Header row removed).ctl" Type="VI" URL="../Sorting/_TypeDefs/TYPE - Sorted Array (Cols in Original Order. Header row removed).ctl"/>
				<Item Name="TYPE - Sorted Array (Cols in Sort Order. Header row removed).ctl" Type="VI" URL="../Sorting/_TypeDefs/TYPE - Sorted Array (Cols in Sort Order. Header row removed).ctl"/>
				<Item Name="TYPE - Sorted Array (Cols in Sort Order. Headers as last row).ctl" Type="VI" URL="../Sorting/_TypeDefs/TYPE - Sorted Array (Cols in Sort Order. Headers as last row).ctl"/>
			</Item>
			<Item Name="Readme.txt" Type="Document" URL="../Sorting/Readme.txt"/>
			<Item Name="Sort 2D String Array.vi" Type="VI" URL="../Sorting/Sort 2D String Array.vi"/>
		</Item>
	</Item>
	<Item Name="xijk_select_best_filter.vi" Type="VI" URL="../xijk_select_best_filter.vi"/>
	<Item Name="Sort 2D String Array - Sort Data.vi" Type="VI" URL="../Sorting/_Subs/Sort 2D String Array - Sort Data.vi"/>
	<Item Name="xijk_select_best_filter_remove_NaN.vi" Type="VI" URL="../xijk_select_best_filter_remove_NaN.vi"/>
	<Item Name="xijk_select_best_filter_calculate_ratio.vi" Type="VI" URL="../xijk_select_best_filter_calculate_ratio.vi"/>
	<Item Name="xijk_select_best_filter_sort.vi" Type="VI" URL="../xijk_select_best_filter_sort.vi"/>
	<Item Name="xijk_search_2D_table.vi" Type="VI" URL="../xijk_search_2D_table.vi"/>
</Library>
